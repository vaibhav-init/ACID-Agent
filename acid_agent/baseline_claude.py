"""Raw Claude Code baseline — the ablation control without ACID machinery.
...
def run_baseline_claude(task: str, ws, tracer=None, run_id=None) -> str:
    prompt = f"{BASELINE_PROMPT}\n\nTask: {task}"
    result = run_claude(prompt, cwd=ws.root)
    if tracer:
        tracer.log(
            "claude_baseline_exec",
